...
FILEpath = glob.glob(os.path.join('../20170609_wiki_756.ucorpus_result', '*.ucorpus'))
# 케냐.txt
FILES = [f.rsplit('\\')[1].split('_')[1].split('(본문)')[0] + '.txt' for f in FILEpath]
...
for filename in FILENAME:
    with open(FILEpath[FILENAME.index(filename)], "r", encoding="utf-8") as f:
            ...
            for n, d in enumerate(data):
                if d != '':
                    ...
                    if len_1 == len_2:
                        ...
                        with open('./test/' + filename + '.txt', "a", encoding="utf-8") as result_file:
